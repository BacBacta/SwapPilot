 ...
 * FstSwap Adapter
 * 
 * FstSwap (FastSwap) is a DEX on BNB Chain with a Uniswap V2-compatible interface.
 * It focuses on fast, low-fee swaps on BSC.
 * 
 * Router Address: Uses standard V2 interface compatible with PancakeSwap liquidity
 */

// FstSwap Router ABI (V2-compatible)
const FSTSWAP_ROUTER_ABI = [
  {
    type: 'function',
    name: 'getAmountsOut',
...
function placeholderSignals(reason: string): RiskSignals {
  return {
    ...
    mevExposure: { level: 'HIGH', reasons: ['fstswap_v2_direct'] },
...
export type FstSwapAdapterConfig = {
  chainId: number;
  rpcUrl: string | null;
...
// FstSwap Router addresses by chain
const ROUTER_ADDRESSES: Record<number, string> = {
  56: '0x10ED43C718714eb63d5aA57B78B54704E256024E', // BSC - Using PancakeSwap router for shared liquidity
};

// WBNB address on BSC
const WBNB = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c';

export class FstSwapAdapter implements Adapter {
  ...
  constructor(config: FstSwapAdapterConfig) {
  ...
  getProviderMeta(): ProviderMeta {
    return {
      ...
      integrationConfidence: this.quoteEnabled() ? 0.7 : 0.4,
      notes: this.quoteEnabled()
        ? 'On-chain quote + swap via FstSwap Router.'
  ...
  async getQuote(request: QuoteRequest): Promise<AdapterQuote> {
    const base: Omit<AdapterQuote, 'raw' | 'normalized'> = {
      ...
      signals: placeholderSignals('fstswap_adapter'),
      deepLink: `https://fstswap.io/swap?inputCurrency=${request.sellToken}&outputCurrency=${request.buyToken}`,
      warnings: [],
      isStub: true,
    };
    ...
    try {
      ...
      const raw = {
        ...
        estimatedGas: 110000, // FstSwap optimized for speed
        feeBps: 25, // 0.25% fee
        route: [request.sellToken, request.buyToken],
      };
  ...
  async buildTx(request: QuoteRequest, quote: AdapterQuote): Promise<BuiltTx> {
    if (!this.quoteEnabled()) {
      throw new Error('FstSwap not configured for this chain');
    }
