...
export function LandioSwapController() {
  ...
  const pendingIntentAutoExecuteRef = useRef(false); // Set to true when AI Intent "Confirm & Swap" is clicked to auto-execute after quotes load
  ...
  useEffect(() => {
    ...
    postQuotes({
      ...
      // Auto-execute swap if triggered from AI Intent "Confirm & Swap"
  ...
  useEffect(() => {
    ...
    const showPreview = (data: {
      ...
      cardHeader.innerHTML = `
        ...
        <span style="font-size:12px;color:var(--text-muted);background:var(--bg-card);border:1px solid var(--border);padding:3px 8px;border-radius:20px;">${badge} ${Math.round(confidence * 100)}% confidence</span>
      `;
      ...
      if (st) {
        ...
        sellRow.innerHTML = `
          ...
          <div style="text-align:right;">
            <span style="font-size:15px;font-weight:700;color:var(--text-primary)">${fmtAmount(sellAmountHuman)} ${st.symbol}</span>
            ${sellUsd ? `<span style="display:block;font-size:12px;color:var(--text-muted)">${sellUsd}</span>` : ''}
          </div>
        `;
      ...
      if (bt) {
        ...
        buyRow.innerHTML = `
          ...
          <div style="text-align:right;">
            <span style="font-size:15px;font-weight:700;color:var(--ok,#00e676)">≈ ${estimatedBuyAmount ? fmtAmount(estimatedBuyAmount) : '—'} ${bt.symbol}</span>
            ${buyUsd ? `<span style="display:block;font-size:12px;color:var(--text-muted)">${buyUsd}</span>` : ''}
          </div>
        `;
      ...
      // Details grid: Route / Gas / Slippage / Network
      ...
      // Fetch route + gas in background and update the grid cells
      ...
      modifyBtn.style.cssText = `flex:1;padding:12px;border-radius:12px;background:transparent;border:1px solid var(--border);font-size:13px;font-weight:600;color:var(--text-muted);cursor:pointer;transition:border-color 0.15s;`;
      ...
      confirmBtn.style.cssText = `flex:2;padding:12px 16px;border-radius:12px;background:var(--accent);border:none;font-size:14px;font-weight:700;color:#000;cursor:pointer;transition:opacity 0.15s,transform 0.1s;letter-spacing:0.01em;`;
      ...
      confirmBtn.onclick = () => {
        // Signal that we should auto-execute the swap once quotes are loaded
        ...
        if (st && sellAmountHuman > 0) {
          ...
          if (fromInput) {
            ...
            // Fire input event with a short delay to let React state settle first
